// ============================================================
// MeiraWatch - script.js (OPTIMIZED SYNC ENGINE v6)
// FITUR: Join langsung tanpa login, support Bunny.net, sync stabil
// ============================================================
...
// ============================================================
// EKSTRAKSI URL (Google Drive, YouTube, Bunny.net)
// ============================================================
...
function isBunnyNetUrl(url) {
    return url.includes('bunnycdn.com') || url.includes('bunny.net');
}
...
document.addEventListener('DOMContentLoaded', loadMoviePoster);

// ============================================================
// JOIN ROOM (LANGSUNG, TANPA LOGIN & SEAT)
// ============================================================
...
function applyHostTick(data) {
    ...
    // Latency estimation (NTP-like)
    if (data.hostSendTime) {
        const rtt = Date.now() - data.hostSendTime;
        syncEngine.latencySamples.push(rtt);
        if (syncEngine.latencySamples.length > 20) syncEngine.latencySamples.shift();
        const sorted = [...syncEngine.latencySamples].sort((a, b) => a - b);
        syncEngine.avgLatency = sorted[Math.floor(sorted.length / 2)] || 0;
    }
...
function stopSyncEngine() {
    ...
    if (syncEngine.hostInterval) { clearInterval(syncEngine.hostInterval);
        syncEngine.hostInterval = null; }
    if (syncEngine.rafId) { cancelAnimationFrame(syncEngine.rafId);
        syncEngine.rafId = null; }
...
// ============================================================
// LOAD VIDEO - SUPPORT BUNNY.NET, DRIVE, YOUTUBE
// ============================================================
...
function loadVideoUrl(url) {
    ...
    const isBunny = isBunnyNetUrl(url);
    ...
    if (ytId) {
        ...
        const proxyUrl = `/gdrive-stream?id=${driveId}`;
        console.log("🎬 Streaming Drive via server proxy:", proxyUrl);
        ...
        isYouTube = false;
        videoEl.style.display = 'block';
        console.log("🎬 Streaming Bunny.net langsung:", url);
        videoEl.src = url;
        videoEl.load();
        let playPromise = videoEl.play();
        if (playPromise !== undefined) {
...
function changeVideo() {
    ...
    if (!rawUrl) {
        alert('Masukkan link Google Drive, YouTube, atau Bunny.net!');
        return;
    }

    socket.emit('video-changed', { url: rawUrl, serverTime: Date.now() });
    loadVideoUrl(rawUrl);
    inputEl.value = '';
}
...
function playStream() {
    ...
    if (!raw) { alert('Masukkan link video terlebih dahulu!'); return; }
...
// Sent to everyone when a host reconnects under a new socket id.
// Quietly sync isHost/hostId without re-showing "you are host/viewer" messages.
socket.on('role-assigned-broadcast', (data) => {
    hostId = data.hostId;
    if (data.hostId === socket.id) {
        isHost = data.isHost;
    } else if (isHost && data.hostId !== socket.id) {
        isHost = false;
    }
    updateControlsVisibility();
});
...
socket.on('video-changed', (data) => {
    ...
    if (url) {
        ...
        loadVideoUrl(url);
        if (!isHost) showSystemMessage('🎬 Video baru dimuat oleh Host');
    }
});
...
function sendMessage() {
    ...
    if (!socket.connected) {
        showSystemMessage('⚠️ Koneksi terputus, pesan belum bisa dikirim. Menunggu menyambung ulang...');
        return;
    }
...
function updateControlsVisibility() {
    ...
    if (isHost) {
        show(uploadSection);
        show(streamSection);
        show(broadcastSection);
        showInline(lockBtn);
        showInline(syncBtn);
        showInline(screenShareBtn);
        participantsSection && (participantsSection.style.display = 'block');
    } else {
        hide(uploadSection);
        hide(streamSection);
        hide(broadcastSection);
        hide(lockBtn);
        hide(syncBtn);
        hide(screenShareBtn);
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    reactionBar.addEventListener('mouseleave', () => { isDown = false;
        reactionBar.classList.remove('dragging');
        reactionBar.style.cursor = 'grab'; });
    reactionBar.addEventListener('mouseup', () => { isDown = false;
        reactionBar.classList.remove('dragging');
        reactionBar.style.cursor = 'grab'; });
    ...
    reactionBar.addEventListener('wheel', (e) => { e.preventDefault();
        reactionBar.scrollLeft += e.deltaY; }, { passive: false });
});

// ============================================================
// CONNECTION HEALTH & ANTI-ZOMBIE RECONNECT
// ============================================================
...
function showConnectionBanner(text, type) {
    ...
    if (!banner) {
        ...
        banner.style.cssText = `
            ...
            font-family: inherit; font-weight: 600; color: #fff;
            transition: background-color 0.3s ease;
        `;
        document.body.appendChild(banner);
    }
    banner.innerText = text;
    if (type === 'error') banner.style.backgroundColor = '#dc2626';
    else if (type === 'warning') banner.style.backgroundColor = '#f59e0b';
    else banner.style.backgroundColor = '#16a34a';
    banner.style.display = 'block';
    if (type === 'success') {
        setTimeout(() => {
            if (banner.style.backgroundColor === 'rgb(22, 163, 74)') {
...
// Re-join the room and resync state after a (re)connection.
// This is the critical fix for "zombie connections": socket.io
// automatically reconnects under the hood with a NEW socket.id,
// but the server has no idea this new socket belongs to a room
// until we explicitly send 'join-room' again.
...
socket.on('connect', () => {
    ...
    if (isReconnecting) {
        showSystemMessage('🔄 Koneksi pulih, menyinkronkan ulang...');
        showConnectionBanner('✅ Koneksi pulih kembali', 'success');
        rejoinRoomIfNeeded();
...
socket.on('disconnect', (reason) => {
    ...
    if (reason === 'io server disconnect') {
        // Server forcibly closed the connection (e.g. kicked) — must reconnect manually.
        showConnectionBanner('⚠️ Terputus dari server, mencoba menyambung ulang...', 'error');
        socket.connect();
    } else {
        showConnectionBanner('⚠️ Koneksi terputus, mencoba menyambung ulang...', 'warning');
    }
});

socket.io.on('reconnect_attempt', (attempt) => {
    showConnectionBanner(`🔄 Mencoba menyambung ulang... (percobaan ${attempt})`, 'warning');
});

socket.io.on('reconnect', () => {
    // Fallback in case 'connect' above doesn't catch it (defensive)
...
socket.io.on('reconnect_failed', () => {
    showConnectionBanner('❌ Gagal menyambung ke server. Periksa koneksi internet Anda.', 'error');
});
...
socket.on('connect_error', () => {
    if (!isReconnecting) {
        isReconnecting = true;
        connectionLostAt = Date.now();
        showConnectionBanner('⚠️ Masalah koneksi, mencoba menyambung ulang...', 'warning');
    }
});

// Detect tab coming back into focus / device waking from sleep.
// Mobile browsers and laptops often suspend timers/sockets when a tab
// is backgrounded for a while, leaving a "zombie" socket that LOOKS
// connected but no longer receives events. Force a check + rejoin.
document.addEventListener('visibilitychange', () => {
    if (document.visibilityState === 'visible' && currentRoom) {
        if (!socket.connected) {
            socket.connect();
        } else {
            // Socket claims to be connected, but verify by re-syncing.
            rejoinRoomIfNeeded();
        }
    }
});
...
window.addEventListener('online', () => {
    if (currentRoom && !socket.connected) {
        showConnectionBanner('🌐 Koneksi internet kembali, menyambung ulang...', 'warning');
        socket.connect();
    }
});

window.addEventListener('offline', () => {
    showConnectionBanner('📵 Tidak ada koneksi internet', 'error');
});

// Periodic watchdog: if we think we're connected but haven't gotten
// any heartbeat/tick from the server in a while (for non-host viewers),
// proactively ping to verify the connection is alive (not a zombie).
setInterval(() => {
...
socket.on('pong-check', () => { /* connection confirmed alive */ });
...
setInterval(() => {
    const el = document.getElementById('syncMonitorSimple');
    if (!el || el.style.display === 'none') return;
    const v = document.getElementById('videoPlayer');
    if (!v) return;
    const drift = typeof syncEngine !== 'undefined' ? (syncEngine.hostTime - v.currentTime) : 0;
    const latency = typeof syncEngine !== 'undefined' ? syncEngine.avgLatency : 0;
    let bufAhead = 0;
    if (v.buffered.length > 0) {
        bufAhead = Math.max(0, v.buffered.end(v.buffered.length - 1) - v.currentTime);
    }
    document.getElementById('smDrift').textContent = drift.toFixed(3) + 's';
    document.getElementById('smRate').textContent = v.playbackRate.toFixed(3) + 'x';
    document.getElementById('smBuffer').textContent = bufAhead.toFixed(1) + 's';
    document.getElementById('smLatency').textContent = latency.toFixed(0) + 'ms';
    const driftEl = document.getElementById('smDrift');
    driftEl.style.color = Math.abs(drift) < 0.15 ? '#4ade80' : Math.abs(drift) < 0.5 ? '#fbbf24' : '#ef4444';
}, 500);

console.log('✅ MeiraWatch siap digunakan! (Sync Engine v6 — Bunny.net support + Join langsung)');